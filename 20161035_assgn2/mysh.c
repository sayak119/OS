...
#define COMMAND_KJOBS 7
#define COMMAND_EXPORT 8
#define COMMAND_UNSET 9
#define COMMAND_OVERKILL 10
...
int get_command_type(char *command) {
    if (strcmp(command, "quit") == 0) {
        ...
        return COMMAND_KILL;
    } else if(strcmp(command, "kjobs") == 0) {
        return COMMAND_KJOBS;
    }else if (strcmp(command, "setenv") == 0) {
        return COMMAND_EXPORT;
    } else if (strcmp(command, "unsetenv") == 0) {
        return COMMAND_UNSET;
    }else if (strcmp(command, "overkill") == 0) {
        return COMMAND_OVERKILL;
     }else {
...
int shell_killjobs(int argc, char **argv) {
    if (argc < 3) {
        printf("usage: killjobs <job no.> <signal no.>\n");
        return -1;
    }
    int signu = atoi(argv[2]);
    pid_t pid;
    int job_id = -1;
...
int shell_overkill(int argc, char **argv) {
    // // if (argc < 3) {
    // //     printf("usage: killjobs <job no.> <signal no.>\n");
    // //     return -1;
    // // }
...
int shell_execute_builtin_command(struct process *proc) {
    int status = 1;
    ...
    switch (proc->type) {
        ...
        case COMMAND_CD:
            ...
            break;
        case COMMAND_JOBS:
            shell_jobs(proc->argc, proc->argv);
            break;
        case COMMAND_FG:
            shell_fg(proc->argc, proc->argv);
            break;
        case COMMAND_BG:
            shell_bg(proc->argc, proc->argv);
            break;
        case COMMAND_KILL:
            shell_kill(proc->argc, proc->argv);
            break;
        case COMMAND_KJOBS:
            shell_killjobs(proc->argc, proc->argv);
            break;
        case COMMAND_EXPORT:
            shell_export(proc->argc, proc->argv);
            break;
        case COMMAND_UNSET:
            shell_unset(proc->argc, proc->argv);
            break;
        case COMMAND_OVERKILL:
            shell_overkill(proc->argc, proc->argv);
            break;
        default:
            status = 0;
            break;
...
int shell_launch_process(struct job *job, struct process *proc, int in_fd, int out_fd, int mode) {
    proc->status = STATUS_RUNNING;
    if (proc->type != COMMAND_EXTERNAL && shell_execute_builtin_command(proc)) {
        return 0;
    }
...
int shell_launch_job(struct job *job) {
    struct process *proc;
    ...
    for (proc = job->root; proc != NULL; proc = proc->next) {
        ...
        if (proc->next != NULL) {
            pipe(fd);
            status = shell_launch_process(job, proc, in_fd, fd[1], pipex);
            close(fd[1]);
            in_fd = fd[0];
        } else {
            int out_fd = 1;
            ...
            status = shell_launch_process(job, proc, in_fd, out_fd, job->mode);
        }
    }

    if (job->root->type == COMMAND_EXTERNAL) {
        if (status >= 0 && job->mode == fgex) {
            remove_job(job_id);
        } else if (job->mode == bgex) {
            print_job(job_id);
        }
    }
...
void shell_print_promt() {
    ...
    printf(COLOR_RED "shell>" COLOR_NONE " ");
}

// void shell_print_welcome() {
//     printf("Welcome to shell by 0456018!\n");
// }
